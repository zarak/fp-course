...
findRight p (ListZipper left a right) =
    let (breakLeft, breakRight) = break p right
     in case (breakLeft, breakRight) of
          (_, Nil) -> IsNotZ -- item not found in left list
          (_, r :. rs) -> if p r then IsZ (ListZipper (reverse breakLeft ++ a :. left) r rs) else IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (l :. ls) a rs) =
    ListZipper ls l (a :. rs)
moveLeftLoop (ListZipper Nil a rs) =
    case reverse (a :. rs) of 
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls a (r :. rs)) =
    ListZipper (a :. ls) r rs
moveRightLoop (ListZipper ls a Nil) =
    case reverse (a :. ls) of 
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (l :. ls) a rs) = 
    IsZ $ ListZipper ls l (a :. rs)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper ls a (r :. rs)) = 
    IsZ $ ListZipper (a :. ls) r rs
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (l :. ls) a rs) = 
    IsZ $ ListZipper (a :. ls) l rs
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper ls a (r :. rs)) = 
    IsZ $ ListZipper ls r (a :. rs)
...
moveLeftN n lz
  | n == 0 = IsZ lz
  | n > 0  = let moved = moveLeft lz
             in
             case moved of
               IsNotZ -> IsNotZ
               IsZ newLz -> moveLeftN (n - 1) newLz
  | otherwise = let moved = moveRight lz 
                in 
                case moved of 
                  IsNotZ -> IsNotZ 
                  IsZ newLz -> moveLeftN (n + 1) newLz
...
moveRightN n lz
  | n == 0 = IsZ lz
  | n > 0  = let moved = moveRight lz
             in
             case moved of
               IsNotZ -> IsNotZ
               IsZ newLz -> moveRightN (n - 1) newLz
  | otherwise = let moved = moveLeft lz 
                in 
                case moved of 
                  IsNotZ -> IsNotZ 
                  IsZ newLz -> moveRightN (n + 1) newLz
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl fa fr) (ListZipper l a r) =
    ListZipper (zipWith (\f a' -> f a') fl l) (fa a) (zipWith (\f a' -> f a') fr r)
