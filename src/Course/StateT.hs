...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) sf ssfa =
      StateT (\s ->
          let ff = runStateT sf s
           in do
               (f, s1) <- ff
               (a, s2) <- runStateT ssfa s1
               pure (f a, s2)
             )
...
instance Monad f => Applicative (OptionalT f) where
  pure a =
      let x = pure (pure a)
       in OptionalT x
  (<*>) (OptionalT f) (OptionalT foa) =
      OptionalT (f >>= (\oab -> onFull (\e -> (e <$>) <$> foa) oab))
      --OptionalT (f >>= (\oab -> foa >>= (\a -> let x = oab <*> a in pure x)))
  --
  --(<*>) optf opta =
...
instance Monad f => Monad (OptionalT f) where
  (=<<) aoptfb (OptionalT foa) = 
      let x = foa >>= g
          g (Full a) = let y = (aoptfb a) in runOptionalT y
          g Empty = pure Empty
       in OptionalT x
